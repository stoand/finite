...
//     parse_fragment(sink, Default::default(), context_name, Vec::new())
//         .from_utf8()
//         .one(bytes)
// }
...
//     // BracketedCode { contents: RefCell<StrTendril> },
//     BracketedCode { contents: String },
...
//                 { |r: &[u8]| NodeData::BracketedCode { contents: String::from_utf8(r.to_vec()).unwrap() } }
//             take_until!("}}") => 
...
//     //     let mut curly_braces = between((token('{'), token('{')), token('}'), str_rm_end);
